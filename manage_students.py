import create_students_table as cst
import sys
...
def ajouter_etudiant_manuel():
    ...
    if cst.etudiant_existe(email=email):
    ...
    id_etudiant, mot_de_passe_genere = cst.ajouter_etudiant(
        nom=nom,
        prenom=prenom,
        email=email,
        telephone=telephone,
...
def verifier_etudiant():
    ...
    if choix == "1":
        ...
        existe = cst.etudiant_existe(id_etudiant=id_etudiant)
    elif choix == "2":
        email = input("Email: ")
        existe = cst.etudiant_existe(email=email)
    else:
...
def ajouter_etudiants_test():
    ...
    for e in etudiants:
        if not cst.etudiant_existe(email=e["email"]):
            # Laisser le mot de passe vide pour qu'il soit généré automatiquement
            id_etudiant, mot_de_passe = cst.ajouter_etudiant(
                nom=e["nom"],
                prenom=e["prenom"],
                email=e["email"],
                telephone=e["tel"]
...
def lister_etudiants():
    ...
    etudiants = cst.obtenir_tous_etudiants()
...
def reinitialiser_mot_de_passe():
    ...
    if not cst.etudiant_existe(id_etudiant=id_etudiant, email=email):
    ...
    succes, nouveau_mot_de_passe = cst.reinitialiser_mot_de_passe(id_etudiant=id_etudiant, email=email)
